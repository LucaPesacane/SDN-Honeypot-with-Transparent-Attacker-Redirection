...
#
# FASE 5: il detector non si limita piu' a segnalare. Alla rilevazione di
#         un port scan il controller installa nel data plane le regole che
#         dirottano l'attaccante verso l'honeypot in modo trasparente.
#
# ---------------------------------------------------------------------------
# Come funziona la redirezione
# ---------------------------------------------------------------------------
# Si tratta di un NAT bidirezionale realizzato con due flow entry simmetriche
# installate sullo switch di ingresso dell'attaccante (priorita' 100):
#
#   ANDATA   match: ip_src=ATT, ip_dst=PROTECTED
#            actions: set ipv4_dst=HONEYPOT, set eth_dst=HONEYPOT_MAC,
#                     output -> porta verso l'honeypot
#
#   RITORNO  match: ip_src=HONEYPOT, ip_dst=ATT
#            actions: set ipv4_src=PROTECTED, set eth_src=PROTECTED_MAC,
#                     output -> porta dell'attaccante
#
# L'attaccante invia a PROTECTED e riceve risposte da PROTECTED: non ha modo
# di accorgersi del dirottamento. Open vSwitch ricalcola automaticamente i
# checksum IP/TCP a seguito delle azioni set_field.
#
# L'ARP non viene toccato (le regole matchano solo eth_type=IP): le richieste
# raggiungono il server reale, quindi la cache ARP dell'attaccante contiene
# il MAC autentico della vittima e il dirottamento resta invisibile anche a
# un controllo con "arp -n".
#
# Limite noto: OpenFlow non puo' riscrivere i numeri di sequenza TCP, quindi
# le sessioni gia' stabilite prima della rilevazione si interrompono. Per il
# port scan non e' rilevante, essendo costituito da sole connessioni nuove.
#
# Recovery: le regole hanno idle_timeout. Cessato l'attacco scadono da sole,
# lo switch notifica il controller con FlowRemoved e l'host torna al
# forwarding ordinario senza alcun intervento esterno.
#
# Uso:
...
# verso s2, su s2 e' la porta di accesso dell'honeypot. Deriva dalla
# topologia (cfr. port_map.json generato da topology.py).
...
PORTSCAN_PORT_THRESHOLD = 20      # derivata dalla baseline: 4x il max (5)
...
REDIRECT_IDLE_TIMEOUT = 60        # s di inattivita' prima del recovery
...
class HoneypotSwitch13(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.suspicious = {}      # ip -> stato
        self.redirected = {}      # ip -> {'since', 'dpid', 'target'}
        self.alert_count = 0
        ...
        if ENABLE_DETECTION:
            self.logger.info("detector: soglia port scan = %d porte / %.0fs",
    ...
    def install_containment(self, datapath):
        """
        L'honeypot puo' solo fare ARP; ogni altro traffico che origina da
        lui viene scartato. Le eccezioni verso gli host dirottati NON sono
        preconfigurate: vengono installate dal controller nel momento in cui
        un host viene classificato come malevolo (install_redirection).
        """
        parser = datapath.ofproto_parser
        ofproto = datapath.ofproto
    ...
    def _packet_in_handler(self, ev):
        ...
        if ip4 is not None:
            # localizzazione dell'host: serve al redirector per sapere su
            # quale switch e porta e' attestato l'attaccante
    ...
    def install_redirection(self, attacker_ip, target_ip=PROTECTED_IP):
        """
        Installa il NAT bidirezionale che dirotta l'attaccante sull'honeypot.
        Restituisce True se le regole sono state effettivamente installate.
        """
        ...
        # L'honeypot deve poter rispondere a QUESTO host, e solo a lui.
    ...
    def _flow_removed_handler(self, ev):
        ...
        esce dalla quarantena e torna al forwarding ordinario. Nessun
        intervento esterno e' necessario.
        """
        msg = ev.msg
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for src in set(agg) | set(self.flow_events):
            ...
            a = agg.get(src, {'pkts': 0, 'bytes': 0, 'flows': 0, 'single': 0})
        ...
        for r in sorted(rows, key=lambda x: -int(x['distinct_dports']))[:3]:
            if int(r['distinct_dports']) > 0:
                self.logger.info(
                    "dpid=%s src=%-11s dports=%-4s newflows/s=%-6s "
                    "avgB=%-7s single=%s%s",
                    r['dpid'], r['src_ip'], r['distinct_dports'],
                    r['new_flows_per_s'], r['avg_bytes_per_flow'],
                    r['single_pkt_flow_ratio'],
                    "  [REDIRECTED]" if r['redirected'] else "")

    @set_ev_cls(ofp_event.EventOFPPortStatsReply, MAIN_DISPATCHER)
    def _port_stats_reply_handler(self, ev):
        pass  # raccolte per la fase 9 (carico sul trunk)
    ...
    def _detect(self, now, features):
        for src, f in features.items():
            if src in DETECTION_WHITELIST:
                continue
            if src in self.redirected:
                continue          # gia' in quarantena

            n_ports = int(f['distinct_dports'])
            n_flows = int(f['active_flows'])

            if n_ports < PORTSCAN_PORT_THRESHOLD:
                continue
            if n_flows < PORTSCAN_MIN_FLOWS:
                continue
            ...
            self._raise_alert(now, src, 'port_scan', f, renewed=bool(prev))
    ...
    def _raise_alert(self, now, src, atype, evidence, renewed=False):
        ...
        self._log_event({
            ...
            'attack_type': atype,
            'renewed': renewed,
            'threshold': PORTSCAN_PORT_THRESHOLD,
            'evidence': {
        ...
        self.logger.warning(
            "*** ALERT #%d  %s da %s  |  porte=%d (soglia %d)  "
            "flussi=%d  avgB=%.0f  single=%.2f  -> %s",
            self.alert_count, atype, src,
            int(evidence['distinct_dports']), PORTSCAN_PORT_THRESHOLD,
            int(evidence['active_flows']),
            float(evidence['avg_bytes_per_flow']),
            float(evidence['single_pkt_flow_ratio']), action)
    ...
    FIELDS = ['timestamp', 'dpid', 'src_ip', 'distinct_dports',
              ...
              'pkts_per_s', 'bytes_per_s', 'avg_bytes_per_flow',
              'single_pkt_flow_ratio', 'packet_in_per_s', 'redirected']
